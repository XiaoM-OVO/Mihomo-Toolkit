...
## 2026-08-31（构建层 v1.6.0）

### ⚙️ 构建层 v1.6.0

#### ✨ 新增

* **订阅抓取容灾**（`fetchRetry` / `fetchTimeout` / `fetchStaleTtl`）：解决个别订阅偶尔拉取失败导致缺节点、需多次刷新的问题——

  1. **失败自动重试**：`fetchRetry` 默认 2（即最多尝试 3 次），仅对可恢复错误（超时/网络抖动/5xx）重试并带递增退避，4xx 等确定性错误直接放弃，避免无效等待；
  2. **容灾降级**：每个订阅按 URL 独立缓存最近一次成功抓取内容（`fetchStaleTtl` 默认保留 24 小时，设为 0 关闭），订阅持续拉取失败时自动复用上次成功数据，节点不消失；
  3. **构建不完整不写缓存**：存在最终失败且无兜底可用的订阅时，整份结果不再写入内存缓存，下次请求强制重新拉取，避免把残缺配置下发客户端；
  4. **内容有效性校验**：拉取成功但解析不出任何节点（如限流页/错误页）同样按失败进入重试/降级流程；`fetchTimeout` 默认 15s 可全局覆盖单次超时，订阅项可再写 `retry` 字段独立覆盖重试次数。

* **自动化测试套件扩充**：新增订阅抓取容灾单测集（重试成功 / 4xx 不重试 / 容灾降级 / 关闭降级 / 失败不写缓存 / 私有字段过滤），用例总数达 29 项（8 个套件全部通过）。

* **订阅信息节点增强**：信息节点显示剩余流量时一并展示套餐总量，总量格式支持到 TB/PB 多级单位；新增订阅级可选配置 `resetDay`（每月第几号重置流量），配置后自动计算"距离重置剩余 X 天"并生成信息节点，方便到期较远的订阅补全重置信息（不配置则沿用原有自动捕捉逻辑）。

* **TypeScript 类型声明补充**（`index.d.ts`）：新增 `fetchRetry` / `fetchTimeout` / `fetchStaleTtl` 配置项声明。

#### 🐛 修复

* **内部私有字段泄漏到最终 YAML**：构建链路注入的 `_subTag` / `_indexPrefix` / `_rawName` 等 `_` 前缀字段会透出到最终生成的 YAML 配置，现于序列化前统一剥离（按 `_` 前缀通用匹配，后续新增私有字段亦自动拦截），输出配置纯净合规。
...
- **AWS 专属打标**（`🛰️`）：CDN 归为 ☁️ 通用标识，Amazon/AWS 打上 `AWS`/🛰️ 标记，不同供应商透明区分。
- **家宽 emoji 保真**：节点名清洗时保留地区国旗与家宽标识 🏠（U+1F3E0），其余 emoji 一律当噪声删除，家宽标签不再被误删。
- **冰岛地区识别**：地区字典新增冰岛 🇮🇸（雷克雅未克）。

#### 🐛 修复

- **AWS 反查误判为落地**：IP 反查 `isCdnOrAnycast()` 的 CDN 关键词命中表补充 `amazon`/`aws`，避免 AWS/骁云等套壳节点被误当作可信落地定位。

### ⚙️ 主脚本 v3.4.1

#### ✨ 新增

- **AWS 专属打标**（`🛰️`）：CDN 统一归为 ☁️ 通用标识，Amazon/AWS 打上 `AWS`/🛰️ 标记，不同供应商透明区分；特征规则同步新增 `AWS`/`Amazon` 命中。
- **家宽 emoji 保真**：节点名清洗时保留地区国旗与家宽标识 🏠（U+1F3E0），其余 emoji 一律当噪声删除，家宽标签不再被误删。
- **冰岛地区识别**：地区字典新增冰岛 🇮🇸（雷克雅未克），北大西洋节点自动归类进欧洲大洲组。

#### 🧹 重构与优化

- **关闭重命名彻底不替换**：`enableNodeRename: false` 时完全继承原节点名，不再将特征词替换为 emoji 图标（移除 `showFeatureIcon` 对关闭分支的显示层替换），行为与配置注释一致。
- **AnyTLS 图标归位**：`anytls` 从「特征 icon」调整到底层协议 icon（🛡️），与 SS/VMess/VLESS 等协议平级展示，避免与家宽等特征图标混淆。
- **AI 目标地区默认权重调整**：`aiPreferredRegions` 默认列表移除 `hk`，AI 分流优先地区收敛为 us/jp/tw/sg/kr/eu。
- **地区识别优先 pure 清洗名**：节点落地地区带以 pure 清洗后的节点名（`proxy.name`）优先匹配，可感知 IP 反查/标准化地区结果，识别不到再回退原始名。
- **DNS merge 模式订阅优先**：`proxy-server-nameserver` 支持从订阅 `nameserver-policy`/`proxy-server-nameserver` 提取补缺，订阅友好性进一步提升。

### 🔨 构建层 v1.4.0

#### ✨ 新增

- **多订阅并行 Fetch 抓取**：将 `builder.js` 中多订阅串行顺序抓取重构为 `Promise.all` 并发并行抓取，多订阅构建耗时大幅缩短 3x ~ 8x（同时保持输出节点确切顺序）。
- **内存 TTL 缓存与容灾降级**（`enableCache` / `cacheTtl`）：引入本地内存缓存机制（默认 TTL 300 秒），高频重复请求秒级响应（2ms）；当远程订阅抓取或网络请求异常时，自动安全降级返回上一次成功的历史缓存，保障客户端不断连。
- **健康检查探活接口**（`/healthz` & `/ping`）：`server.js` 与 `worker.js` 新增轻量探活路由，响应 200 JSON（含 `status`, `version`, `uptime`），对 VPS 运维监控（Uptime Kuma / Docker / K8s）极度友好。
- **自动化测试套件与 CI 流水线**：添加基于 Node.js 原生 `node:test` 的 14 项单元/集成测试用例，并配置 GitHub Actions 多版本 Node (18/20/22) CI 自动构建流水线。
...
- **未知节点重命名冲突**：未识别地区节点统一套模板导致重名报错，改为保留原名直接进 `🗑️ 未知识别` 组。
...
- **禁止下载节点误丢入下载池**：pure 不再提前擦除「禁止下载/No BT」等特征文字，保留供 toolkit 先识别后擦除，统一行为链路。
- **IP 批量查询间隔硬编码**：`ipApiBatchDelay` 配置项未被实际引用，硬编码 1000ms 改为正确读取配置值。
- **URI 节点白名单匹配失效**：标签字段化后 URI 节点名不含 tag，白名单 `includes` 匹配失败。补 `_subTag` 精确匹配分支。

#### 🧹 重构与优化

- **标签字段化读取**：`getAirportTag` 加 `proxy` 参数，优先读 `proxy._subTag` 字段，消除硬编码 `[]` 依赖。split/单跑模式保留关键词 + 正则双轨兜底。
- **日志系统统一**：pure 日志样式统一为 `[模块] DBG/INFO/WARN/ERR 消息` 格式；去重日志格式统一为 `🧽 [去重] 「原名」→ 与「现有」重复，已移除`；标签显示优化为 `🏷️xxx |` 格式。
- **删除废弃配置项**：移除 `enableIpMetadata` 及未使用字段 `_ipProxy/_ipHosting/_ipMobile`。
