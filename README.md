...
> ⚠️ **兼容性提醒**：本项目**仅在 Clash Verge Rev 上完整测试**。理论上任何使用 Mihomo 内核的客户端都能运行，但未经实测，不保证其他客户端的兼容性。
...
### 1. 增减地区识别 (Region)
若您的节点列表包含特殊地区（如：泰国、土耳其），请修改脚本顶部的 `regionReplacements` 数组：
```js
// 在数组中添加一行，注意正则关键词和图标
...
### 2. 调整 AI 服务的节点优先级
脚本会自动根据节点存复情况生成 AI 安全组。若想调整**自动排列的顺序**（例如让新加坡排在最前面），请修改 `aiSafeRegions` 数组的顺序：
```js
// 谁在前面，AI 策略组的下拉列表就优先排谁
const aiSafeRegions = ["🇸🇬 新加坡节点", "🇺🇸 美国节点", "🇯🇵 日本节点", ...];
```

### 3. 定义低倍率节点 (防偷跑)
脚本会自动识别并隔离低倍率节点至“下载池”。默认阈值是 **0.99**。若想微调（例如 0.5 倍以下算低倍率），修改此处逻辑：
```js
// 找到这行，修改 < 1 的数值
let isLowMulti = multiStr && parseFloat(multiStr.slice(1)) < 0.5;
```
...
### 5. 更新规则集
本脚本默认使用 `MetaCubeX` 的规则集。若需切换其他源，只需修改 `repo` 变量和 `rules` 数组。
